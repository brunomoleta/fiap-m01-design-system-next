...
type EditTransactionStore = {
  ...
  updateTransaction: (transaction: TransactionType, callback: () => void) => Promise<void>;
};
...
const useEditTransactionStore = create<EditTransactionStore>((set, get) => ({
  ...
  updateTransaction: async (transaction: TransactionType, callback: () => void) => {
    ...
    try {
      ...
      const adjustedValue = transactionType === 'deposito' ? Math.abs(nValue) : -Math.abs(nValue);
