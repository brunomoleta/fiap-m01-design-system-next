...
const TransactionForm = ({refreshExtract}: Props) => {
  const {loading} = useUtilsStore()
  const {transactionType, setTransactionType, fetchData, setTransactionValue, transactionValue} =
    useTransactionStore()

  return (<>
    <form onSubmit={() => fetchData(refreshExtract)} className="flex flex-col gap-spacing-xl grow order-2">
      <Select
      ...
      />
      <Button variant="secondary" onClick={() => fetchData(refreshExtract)} disabled={loading}>
        Concluir transação
      </Button>
    </form>
  </>);
